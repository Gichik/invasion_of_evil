...
function item_note_dungeon_jeepers_one:CastFilterResultTarget(hTarget)
	--print("Error")
	if IsServer() then
...
function item_note_dungeon_jeepers_one:GetCustomCastErrorTarget(hTarget)
	--print("Error")
	if IsServer() then
...
function item_note_dungeon_jeepers_one:OnSpellStart()
	...
	--print("OnSpellStart")
		...
		if hTarget:GetUnitName() == "npc_vern_base" then
			...
			main:CreateDrop("item_note_dungeon_jeepers_two", hCaster:GetAbsOrigin())
		end
	end
end