...
if modifier_vortex_axe_second == nil then
    modifier_vortex_axe_second = class({})
end

--rotate = 0

function modifier_vortex_axe_second:IsHidden()
	return false
end

function modifier_vortex_axe_second:IsPurgable()
	return false
end

function modifier_vortex_axe_second:GetTexture()
    return "custom_folder/vortex"
end

function modifier_vortex_axe_second:RemoveOnDeath()
	return true
end

function modifier_vortex_axe_second:DeclareFunctions()
    local funcs = {
...
function modifier_vortex_axe_second:GetOverrideAnimation()	
	return ACT_DOTA_CAPTURE
end

function modifier_vortex_axe_second:GetAbilityDamageType()	
	return DAMAGE_TYPE_PHYSICAL
end

function modifier_vortex_axe_second:GetAbilityDamage()	
	return self.damage
end

function modifier_vortex_axe_second:GetModifierConstantManaRegen()	
	return -self.manaCost
end


function modifier_vortex_axe_second:OnOrder(data)
...
function modifier_vortex_axe_second:OnCreated(data)
	self.manaCost = 20
	...
	if IsServer() then
		...
		self.rotateMax = 12
		self.think = 0.03
		self.damage = (2*self.parent:GetStrength()*self.rotateMax*self.think) or 0
...
function modifier_vortex_axe_second:OnIntervalThink()
...
function modifier_vortex_axe_second:OnDestroy()
	if IsServer() then
		if self.blade_fury_spin_pfx then
			ParticleManager:DestroyParticle(self.blade_fury_spin_pfx, false)
			ParticleManager:ReleaseParticleIndex(self.blade_fury_spin_pfx)
...
function modifier_vortex_axe_second:PlayAnimation()
...
function modifier_vortex_axe_second:ShouldMove(VectorA, VectorB)
...
function modifier_vortex_axe_second:ApplyDamage()
