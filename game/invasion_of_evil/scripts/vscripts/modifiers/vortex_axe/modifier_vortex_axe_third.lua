...
if modifier_vortex_axe_third == nil then
    modifier_vortex_axe_third = class({})
end

--rotate = 0

function modifier_vortex_axe_third:IsHidden()
	return false
end

function modifier_vortex_axe_third:IsPurgable()
	return false
end

function modifier_vortex_axe_third:GetTexture()
    return "custom_folder/vortex"
end

function modifier_vortex_axe_third:RemoveOnDeath()
	return true
end

function modifier_vortex_axe_third:DeclareFunctions()
    local funcs = {
...
function modifier_vortex_axe_third:GetOverrideAnimation()	
	return ACT_DOTA_CAPTURE
end

function modifier_vortex_axe_third:GetAbilityDamageType()	
	return DAMAGE_TYPE_PHYSICAL
end

function modifier_vortex_axe_third:GetAbilityDamage()	
	return self.damage
end

function modifier_vortex_axe_third:GetModifierConstantManaRegen()	
	return -self.manaCost
end


function modifier_vortex_axe_third:OnOrder(data)
...
function modifier_vortex_axe_third:OnCreated(data)
	self.manaCost = 30
	...
	if IsServer() then
		...
		self.rotateMax = 12
		self.think = 0.03
		self.damage = (3*self.parent:GetStrength()*self.rotateMax*self.think) or 0
...
function modifier_vortex_axe_third:OnIntervalThink()
...
function modifier_vortex_axe_third:OnDestroy()
	if IsServer() then
		if self.blade_fury_spin_pfx then
			ParticleManager:DestroyParticle(self.blade_fury_spin_pfx, false)
			ParticleManager:ReleaseParticleIndex(self.blade_fury_spin_pfx)
...
function modifier_vortex_axe_third:PlayAnimation()
...
function modifier_vortex_axe_third:ShouldMove(VectorA, VectorB)
...
function modifier_vortex_axe_third:ApplyDamage()
