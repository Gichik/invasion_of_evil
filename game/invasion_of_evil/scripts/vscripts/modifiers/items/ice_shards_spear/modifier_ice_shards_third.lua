...
if modifier_ice_shards_third == nil then
    modifier_ice_shards_third = class({})
end


function modifier_ice_shards_third:IsHidden()
	return false
end

function modifier_ice_shards_third:IsPurgable()
	return false
end

function modifier_ice_shards_third:GetTexture()
    return "winter_wyvern_splinter_blast"
end

function modifier_ice_shards_third:RemoveOnDeath()
	return true
end

function modifier_ice_shards_third:DeclareFunctions()
    local funcs = {
...
function modifier_ice_shards_third:GetAbilityDamageType()	
	return DAMAGE_TYPE_MAGICAL
end


function modifier_ice_shards_third:GetModifierConstantManaRegen()	
	return -self.manaCost
end


function modifier_ice_shards_third:GetModifierAttackSpeedBonus_Constant()	
	return self.attackSpeedBonus or 0
end

function modifier_ice_shards_third:OnCreated(data)
	self.manaCost = 30
...
function modifier_ice_shards_third:OnDestroy()
	if IsServer() then
...
function modifier_ice_shards_third:OnIntervalThink()
...
function modifier_ice_shards_third:OnAttackLanded(data)
	if IsServer() then
		if self:GetParent() then
...
function modifier_ice_shards_third:CreateShards(hTarget)
	if self:GetParent() then
